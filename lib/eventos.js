  ...
  traduzir(evento) {
    ...
    switch (name) {
      ...
      case "CarteiraVinculada": {
        const pendente = BigInt(args.creditoPendente ?? 0n);
        return {
          ...
          detalhes: [
            pendente > 0n
              ? `${brl(pendente)} pendentes de saque (apurados antes do cadastro)`
              : "indenizacoes futuras vao direto para esta carteira"
          ]
        };
      }

      case "CreditoRetidoSacado":
        return {
          rotulo: "PENDENTES SACADOS",
          cor: "verde",
          resumo:
            `passageiro ${curto(args.hashCpf)} · ${brl(args.valor)} depositados em ` +
            `${curto(args.carteira)}`,
          detalhes: ["a partir daqui todo pagamento e automatico"]
        };
